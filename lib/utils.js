...
function renderPost(dir, file) {
    var content = fs.readFileSync(file).toString(),
        post = parseSourceContent(content),
        config = loadConfig(dir);
    post.content = markdownToHTML(post.source);
    post.layout = post.layout || 'post';
    var html = renderFile(path.resolve(dir, '_layout',
        post.layout + '.html'), { post: post, config: config });
    return html;
}
...
function renderIndex(dir) {
    ...
    var config = loadConfig(dir),
        html = renderFile(path.resolve(dir, '_layout',
        'index.html'), { posts: list, config: config });

    return html;
}

function loadConfig(dir) {
    var content = fs.readFileSync(path.resolve(dir, 'config.json'))
        .toString(),
